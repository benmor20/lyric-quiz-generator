...
def possible_titles(title):
    ...
    if ' (' in title:
        ...
        titles.append(title[start + 1:end])
    if ' - ' in title:
        titles.extend(title.split(' - '))  # Assuming there's only one...
    return titles


def possible_artists(artists):
    return sorted(set(a for a in _possible_artists(artists) if len(a) > 0),
...
def possible_phrases(phrase):
    return list(_possible_phrases(phrase.lower(), REPLACEMENTS))
